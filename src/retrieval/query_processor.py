...
class QueryProcessor(LoggerMixin):
    ...
    def verify_content_relevance(self, query: str, content: str) -> bool:
        ...
        system_instruction = (
            "You are a Relevance Verifier. Your job is to check if a text snippet "
            "actually contains the answer to a user's request, or if it's a 'False Positive'.\n\n"
            "Rules for 'Irrelevant' (False Positive):\n"
            "1. Text explicitly says the content (proof/definition) is OMITTED or left as an exercise.\n"
            "2. Text refers to another book/chapter for the actual content WITHOUT providing it.\n"
            "3. Text mentions the theorem but only discusses applications/examples WITHOUT defining or proving it (unless user asked for examples).\n"
            "4. Text defines a DIFFERENT theorem (e.g. 'Theorem 9.3') as a consequence of the requested theorem, but does not define the requested theorem itself.\n"
            "5. Text is just a table of contents or index.\n\n"
            "Rules for 'Relevant' (True Positive):\n"
            "1. If text contains the DEFINITION or STATEMENT of the theorem, it is RELEVANT.\n"
            "2. If text contains the PROOF or DERIVATION of the theorem, it is RELEVANT.\n"
            "3. If text contains specific PROPERTIES requested by user, it is RELEVANT.\n\n"
