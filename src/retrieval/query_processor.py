...
class QueryProcessor(LoggerMixin):
    ...
    def verify_content_relevance(self, query: str, content: str) -> bool:
        ...
        system_instruction = (
            ...
            "RULES:\n"
            "A. IF USER WANTS A DEFINITION / EXPLANATION / CONCEPT:\n"
            "   - **RELEVANT** if the text defines, states, or explains the concept/theorem.\n"
            "   - **IGNORE** phrases like 'proof is omitted', 'beyond the scope', 'left as exercise'. These do NOT make the text irrelevant for a definition query.\n"
            "   - **IRRELEVANT** only if the text mentions the term but DOES NOT define it (e.g., 'Section 5 discusses CLT applications' without stating CLT).\n\n"
            "B. IF USER WANTS A PROOF / DERIVATION:\n"
            "   - **RELEVANT** if the text contains the mathematical steps/derivation.\n"
            "   - **IRRELEVANT** if the text explicitly says 'proof omitted', 'see reference X', or only gives the statement.\n\n"
            "   - **RELEVANT** if the text provides a worked example or problem.\n\n"
            "   - **RELEVANT** if the text is a **SECTION HEADER** or **TITLE** matching the query term (e.g., 'Chapter 5: Central Limit Theorem'), even if the immediate text following it is introductory or generic.\n"
            "CRITICAL EXAMPLES:\n"
            "Query: 'CLT definition'\nText: 'The CLT states that... Proof is omitted.' -> RELEVANT (Definition is present, user didn't ask for proof)\n"
            "Query: 'CLT proof'\nText: 'The CLT states that... Proof is omitted.' -> IRRELEVANT (User asked for proof, but it's missing)\n"
            "Query: 'CLT definition'\nText: 'Section 8.3 The Central Limit Theorem. Enter probabilities...' -> RELEVANT (Strong topic match/Header, even if text is noise)\n\n"
            "OUTPUT:\n"
